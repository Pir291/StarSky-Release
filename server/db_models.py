...
def init_full_db():
    ...
    with get_db() as conn:
        ...
        # Удаляем тестового пользователя и его данные если он есть
        cursor = conn.cursor()
        cursor.execute("SELECT Id FROM Users WHERE Username = 'test_user'")
        test_row = cursor.fetchone()
        if test_row:
            test_id = test_row[0]
            cursor.execute("DELETE FROM Messages WHERE UserId = ?", (test_id,))
            cursor.execute("DELETE FROM Sessions WHERE UserId = ?", (test_id,))
            cursor.execute("DELETE FROM AuthCodes WHERE UserId = ?", (test_id,))
            cursor.execute("DELETE FROM Users WHERE Id = ?", (test_id,))
            conn.commit()
            print("🗑️ Тестовый пользователь удалён")
